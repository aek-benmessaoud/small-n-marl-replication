...
## Repository layout
    ...
    paper/
        src/paper.tex        manuscript (IEEEtran)
        figures/             PDF figures generated by scripts/fig_*.py
        preregistration_2026-09-19.md
    src/
    ...
    configs/
        mappo_chaoU.yaml    default campaign config (written into each run)
    scripts/
        launch/             campaign drivers and Windows launchers (.cmd)
        analysis/           aggregation/evaluation/repro helpers (.py)
        figures/            figure-generation scripts (fig_*.py)
    tests/
