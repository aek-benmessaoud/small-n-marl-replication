...
from scripts.agg_robustness import bootstrap_ci
...
def main():
    ...
    for mode, keys, emp in [("simult", ["simult_loc"], ""),
        ...
        for key in keys:
            ...
            print(f"  {key:<10} slowΔ={d.mean():+.4f} wins={wins}/{len(d)} "
                  f"{int((1-args.alpha)*100)}%CI=[{lo:+.4f},{hi:+.4f}] {sign}"
                  f"   |  fastΔ={fd.mean():+.4f} wins={int(np.sum(fd>0))}/"
                  f"{len(fd)} CI=[{flo:+.4f},{fhi:+.4f}]")
            for i, s in enumerate(seeds):
                print(f"      seed {s:<3} slow intr={a[i]:.5f} "
                      f"noi={b[i]:.5f} Δ={d[i]:+.5f}   |  fast Δ={fd[i]:+.5f}")
